...
function createApiClient(config: AxiosRequestConfig, options: IFetcherOptions) {
  ...
  const apiClient = axios.create({
    headers: {
      ...
      "x-tenant-id": Config.NETWORK_CONFIG.X_TENANT_ID,
    },
...
function returnResponseData<T>(
  ...
  // Handle direct responses (like auth endpoints that return token directly)
  if (
    response.data &&
...
function returnResponseDataWithMetaData<T>(
  ...
  if (
    ...
    // Create a fake metadata since we're handling a direct response
    resolve({
